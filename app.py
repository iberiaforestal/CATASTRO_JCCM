...
def cargar_shapefile_local(province, municipality):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for ext in exts:
            ...
            url = f"{base_url}{province}/{municipality}/{filename}"
            try:
    ...
    try:
        ...
        for province in provinces:
            ...
            if not os.path.exists(province_path):
                continue
            municipalities = sorted([d for d in os.listdir(province_path) if os.path.isdir(os.path.join(province_path, d))])
            for municipality in municipalities:
                gdf = cargar_shapefile_local(province, municipality)
                if gdf is None:
                    continue
                ...
                if not seleccion.empty:
                    ...
                    return province, municipality, masa, parcela, parcela_gdf
...
if modo == "Por parcela":
    province_sel = st.selectbox("Provincia", provinces)
    province_path = os.path.join(base_dir, province_sel)
    if os.path.exists(province_path):
        municipalities = sorted([d for d in os.listdir(province_path) if os.path.isdir(os.path.join(province_path, d))])
        municipio_sel = st.selectbox("Municipio", municipalities)
        
        gdf = cargar_shapefile_local(province_sel, municipio_sel)
        ...
        if gdf is not None:
            ...
            if parcela.geometry.geom_type.isin(['Polygon', 'MultiPolygon']).all():
                ...
                st.write(f"Provincia: {province_sel}")
            ...
            st.error(f"No se pudo cargar el shapefile para el municipio: {municipio_sel} en {province_sel}")
    else:
        st.error(f"No se encontró la carpeta para la provincia: {province_sel}")
...
with st.form("formulario"):
    if modo == "Por coordenadas":
        ...
        if x != 0.0 and y != 0.0:
            province_sel, municipio_sel, masa_sel, parcela_sel, parcela = encontrar_municipio_poligono_parcela(x, y)
            if municipio_sel != "N/A":
                st.success(f"Parcela encontrada: Provincia: {province_sel}, Municipio: {municipio_sel}, Polígono: {masa_sel}, Parcela: {parcela_sel}")
            else:
